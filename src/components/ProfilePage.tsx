        ...
        <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden">
					<div className="bg-white rounded-xl shadow-md p-6">
						<div className="flex items-center mb-4">
							...
							{isEditing ? <input type="text" value={editedName} onChange={(e) => setEditedName(e.target.value)} className="text-xl font-semibold text-gray-900 border-b-2 border-blue-500 focus:outline-none" /> : <h3 className="text-xl font-semibold text-gray-900">{profile.name}</h3>}
							<button onClick={() => (isEditing ? handleSave() : setIsEditing(true))} className="ml-auto text-blue-500 hover:text-blue-600 focus:outline-none" aria-label={isEditing ? 'Save profile name' : 'Edit profile name'}>
